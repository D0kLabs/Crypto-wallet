...
import com.d0klabs.cryptowalt.data.LeafExpr.IfStmt;
...
public class Tree extends LeafExpr.Node implements InstructionVisitor, Opcode {
    ...
    LeafExpr.OperandStack stack; // The program stack for this block
     ...
     *            Block.
     */
    public Tree(final Block block, final LeafExpr.OperandStack predStack) {
        this.block = block;
        ...
        stack = new LeafExpr.OperandStack();

        stmts = new StmtList();

        // The first statement in the Tree is the label indicating the start
        // of the basic Block.
        appendStmt(new LeafExpr.LabelStmt(block.label()));

        // Make a copy of predStack
        for (int i = 0; i < predStack.size(); i++) {
            final LeafExpr.Expr expr = predStack.get(i);
            final LeafExpr.Expr copy = (LeafExpr.Expr) expr.clone();
            copy.setDef(null);
            stack.push(copy);
        }
    }
     ...
     */
    public void removeStmt(final LeafExpr.Stmt stmt) {
        stmts.remove(stmt);
    }
    ...
    public void removeLastStmt() {
        ...
        while (iter.hasPrevious()) {
            final LeafExpr.Stmt s = (LeafExpr.Stmt) iter.previous();

            if (s instanceof LeafExpr.LabelStmt) {
                continue;
            }
    ...
    class StmtList extends LinkedList {
        ...
        public void clear() {
            ...
            while (iter.hasNext()) {
                ((LeafExpr.Stmt) iter.next()).cleanup();
            }
        ...
        public Object set(final int index, final Object element) {
            if (index < size()) {
                final LeafExpr.Stmt s = (LeafExpr.Stmt) get(index);
        ...
        public ListIterator listIterator(final int index) {
            ...
            return new ListIterator() {
                ...
                public void add(final Object obj) {
                    ((LeafExpr.Stmt) obj).setParent(Tree.this);
                    last = null;
                    iter.add(obj);
                }
     ...
     */
    public LeafExpr.OperandStack stack() {
        return stack;
    }
     ...
     */
    public void addStmtAfter(final LeafExpr.Stmt stmt, final LeafExpr.Stmt after) {
        if (Tree.DEBUG) {
        ...
        while (iter.hasNext()) {
            final LeafExpr.Stmt s = (LeafExpr.Stmt) iter.next();
     ...
     */
    public void addStmtBefore(final LeafExpr.Stmt stmt, final LeafExpr.Stmt before) {
        if (Tree.DEBUG) {
     ...
     *            The statement to add.
     */
    public void prependStmt(final LeafExpr.Stmt stmt) {
        final ListIterator iter = stmts.listIterator();
    ...
    private void saveStack() {
        ...
        for (int i = 0; i < stack.size(); i++) {
            final LeafExpr.Expr expr = stack.get(i);
            ...
            if (Tree.USE_STACK) {
                ...
                if (!(expr instanceof StackExpr)
                    ...
                    final LeafExpr.Stmt store = new ExprStmt(new StoreExpr(target, expr,
                            expr.type()));
     ...
     * statement's parent.
     */
    private void appendStmt(final LeafExpr.Stmt stmt) {
        if (Tree.DEBUG) {
     ...
     */
    public void addStmt(final LeafExpr.Stmt stmt) {
        saveStack();
        appendStmt(stmt);
    }
     ...
     * @see JumpStmt
     */
    public void addStmtBeforeJump(final LeafExpr.Stmt stmt) {
        final LeafExpr.Stmt last = lastStmt();
    ...
    public void addLabel(final Label label) {
        ...
        addStmt(new LeafExpr.LabelStmt(label));
    }
    ...
    private void addInst(final Instruction inst, final boolean saveValue) {
        if (Tree.DEBUG) {
            ...
            for (int i = 0; i < stack.size(); i++) {
                final LeafExpr.Expr exp = stack.peek(i);
    ...
    public void visit_ldc(final Instruction inst) {
        ...
        final LeafExpr.Expr top = new ConstantExpr(value, type);
        stack.push(top);
    }
    ...
    public void visit_iload(final Instruction inst) {
        ...
        final LeafExpr.Expr top = new LocalExpr(operand.index(), Type.INTEGER);
        stack.push(top);
    }

    public void visit_lload(final Instruction inst) {
        final LocalVariable operand = (LocalVariable) inst.operand();
        final LeafExpr.Expr top = new LocalExpr(operand.index(), Type.LONG);
        stack.push(top);
    }

    public void visit_fload(final Instruction inst) {
        final LocalVariable operand = (LocalVariable) inst.operand();
        final LeafExpr.Expr top = new LocalExpr(operand.index(), Type.FLOAT);
        stack.push(top);
    }
    ...
    public void visit_iaload(final Instruction inst) {
        final LeafExpr.Expr index = stack.pop(Type.INTEGER);
        final LeafExpr.Expr array = stack.pop(Type.INTEGER.arrayType());
        final LeafExpr.Expr top = new ArrayRefExpr(array, index, Type.INTEGER,
                Type.INTEGER);
        stack.push(top);
    }
    ...
    public void visit_laload(final Instruction inst) {
        final LeafExpr.Expr index = stack.pop(Type.INTEGER);
        final LeafExpr.Expr array = stack.pop(Type.LONG.arrayType());
        final LeafExpr.Expr top = new ArrayRefExpr(array, index, Type.LONG, Type.LONG);
        stack.push(top);
    }

    public void visit_faload(final Instruction inst) {
        final LeafExpr.Expr index = stack.pop(Type.INTEGER);
        final LeafExpr.Expr array = stack.pop(Type.FLOAT.arrayType());
        final LeafExpr.Expr top = new ArrayRefExpr(array, index, Type.FLOAT, Type.FLOAT);
        stack.push(top);
    }
    ...
    public void visit_getstatic(final Instruction inst) {
        ...
        try {
            ...
            final EditorContext.FieldEditor e = context.editField(field);
    ...
    public void visit_ifnonnull(final Instruction inst) {
        ...
        final LeafExpr.Expr left = stack.pop(Type.OBJECT);
        final Block t = (Block) block.graph().getNode(inst.operand());
        addStmt(new LeafExpr.IfZeroStmt(LeafExpr.IfStmt.NE, left, t, next));
    }
    ...
    public void visit_rc(final Instruction inst) {
        ...
        final LeafExpr.Expr object = stack.peek(depth.intValue());
    ...
    public void visit_aupdate(final Instruction inst) {
        ...
        if (Tree.AUPDATE_FIX_HACK) {
            ...
            if (depth.intValue() == 1) {
                final LeafExpr.Expr object = stack.peek();
    ...
    public void visit_aswizzle(final Instruction inst) {
        final LeafExpr.Expr index = stack.pop(Type.INTEGER);
        final LeafExpr.Expr array = stack.pop(Type.OBJECT.arrayType());
    ...
    public void visit_aswrange(final Instruction inst) {
        final LeafExpr.Expr end = stack.pop(Type.INTEGER);
        final LeafExpr.Expr start = stack.pop(Type.INTEGER);
        final LeafExpr.Expr array = stack.pop(Type.OBJECT.arrayType());
