...
public class MethodEditor implements Opcode {
    ...
    private int munchCode(byte[] code, int index, int[][] targets, int[][] lookups) {
        ...
        switch(opcode) {
            ...
            case 198:
            case 199:
                target = Instruction.toShort(code[index + 1], code[index + 2]);
            ...
            case 170:
                ...
                j += 4;
                value = Instruction.toInt(code[j], code[j + 1], code[j + 2], code[j + 3]);
                j += 4;
                npairs = Instruction.toInt(code[j], code[j + 1], code[j + 2], code[j + 3]);
                j += 4;
            ...
            case 200:
            case 201:
                target = Instruction.toInt(code[index + 1], code[index + 2], code[index + 3], code[index + 4]);
                targets[index] = new int[1];
                targets[index][0] = index + target;
        }
            ...
            while(true) {
                ...
                if (PRESERVE_DEBUG && arr.length < 65536) {
                    ...
                    for(int i = 0; i < vars.size(); ++i) {
                        ...
                        if (end < end) {
                            locals[i] = new LocalDebugInfo(end, end - end, cp.addConstant(1, entry.var.name()), cp.addConstant(1, entry.var.type().descriptor()), entry.var.index());
                        }
                    }
                    ...
                    for(iter = this.lineNumbers.iterator(); iter.hasNext(); lines[start++] = new LineNumberDebugInfo(array.labelIndex(line.label), line.lineNumber)) {
                        line = (MethodEditor.LineNumberEntry)iter.next();
                    }

                    this.methodInfo.setLineNumbers(lines);
                } else {
                    this.methodInfo.setLineNumbers((LineNumberDebugInfo[])null);
                    this.methodInfo.setLocals((LocalDebugInfo[])null);
                }
